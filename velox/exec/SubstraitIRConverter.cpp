 ...
 if (sRead.has_virtual_table()) {
   ...
   for(int row = 0; row<numRows; row++){
     for (int col = 0; col<tmpSize; col++){
       ...
       std::cout<<"the value for row: "<< row<<"col: "<<col <<"is :"<<sRead.virtual_table().values(row).fields(col).i32()<<std::endl;
     }
   }
   //TODO debug end
   ...
   std::cout<<"In S to V, the tmpRowNum is :"<<tmpRowNum<<"the rowVectorChildSize is :"<<rowVectorChildSize<<std::endl;

   for (int row = 0; row < numRows; row++) {
     for (int col = 0; col < rowVectorChildSize; col++) {
       auto children = rowVector->childAt(col)->values();
       ...
       switch (rowVector->childAt(col)->typeKind()) {
         ...
         case velox::TypeKind::SMALLINT: {
           std::cout << *children->asMutable<int16_t>() << std::endl;
           break;
         }
       }
       // std::cout <<*children->asMutable<int32_t>()<<std::endl;
...
void SubstraitVeloxConvertor::transformVValuesNode(
 ...
 for(int row = 0; row<ttt; row++){
   for (int col = 0; col<tmpSize; col++){
     ...
     std::cout<<"the value for row: "<< row<<"col: "<<col <<"is :"<<sVirtualTable->mutable_values(row)->mutable_fields(col)->i32()<<std::endl;
   }
 }
}
