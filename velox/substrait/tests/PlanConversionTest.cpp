...
class PlanConversionTest : public virtual HiveConnectorTestBase,
 ...
 protected:
  ...
  /// Create a Fixed-width type of Vector without Null values.
  template <typename T>
  ...
  /// Create a String type of Vector without Null values.
  ...
  /// This test will firstly generate mock TPC-H lineitem ORC file. Then,
  /// Velox's computing will be tested based on the generated ORC file. Input:
  /// Json file of the Substrait plan for the below modified TPC-H Q6 query:
  ///
  ///  select sum(l_extendedprice*l_discount) as revenue from lineitem where
  ///  l_shipdate_new >= 8766 and l_shipdate_new < 9131 and l_discount between
  ///  .06 - 0.01 and .06 + 0.01 and l_quantity < 24
  void queryTest(const std::string fileName) {
    ...
    std::string subPlanPath = veloxPath + "/velox/substrait/tests/" + fileName;
...
///  Tested Velox computings include: TableScan  + Project + Aggregate
///  Output: the Velox computed Aggregation result
TEST_P(PlanConversionTest, FilterNotPushDown) {
  queryTest("subFilter.json");
}
