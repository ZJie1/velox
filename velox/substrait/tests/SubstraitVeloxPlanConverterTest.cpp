...
*/
  ...
  void assertVeloxSubstraitRoundTripProject(
    ...
    std::shared_ptr<const PlanNode> vPlan2 = s2VPlanConvertor->toVeloxPlan(*sPlan);
    /*std::shared_ptr<const PlanNode> vPlan2 =
        s2VPlanConvertor->substraitIRToVelox(*sPlan);*/
    auto mesage2 = vPlan2->toString(true, true);
    LOG(INFO)
...
/*TEST_F(SubstraitVeloxPlanConvertorTest, veloxSubstraitRoundTripValuesNode) {
  std::vector<RowVectorPtr> vectors;
...
TEST_F(SubstraitVeloxPlanConvertorTest, veloxToSubstraitValuesNode) {
  std::vector<RowVectorPtr> vectors;
  vectors = makeVector(3, 4, 2);
  createDuckDbTable(vectors);
...
TEST_F(SubstraitVeloxPlanConvertorTest, veloxSubstraitRoundTripProjectNode) {
  std::vector<RowVectorPtr> vectors;
  vectors = makeVector(3, 4, 2);
  createDuckDbTable(vectors);
...
TEST_F(SubstraitVeloxPlanConvertorTest, veloxSubstraitRoundTripFilterNode) {
  std::vector<RowVectorPtr> vectors;
  vectors = makeVector(3, 4, 2);
  createDuckDbTable(vectors);
...
TEST_F(SubstraitVeloxPlanConvertorTest, veloxSubstraitRoundTripFilterProject) {
  std::vector<RowVectorPtr> vectors;
  vectors = makeVector(3, 4, 2);
  createDuckDbTable(vectors);
...
TEST_F(SubstraitVeloxPlanConvertorTest, filterProjectFused) {
  std::vector<RowVectorPtr> vectors;
  vectors = makeVector(3, 4, 2);
  createDuckDbTable(vectors);
